...
public class CMSOperationsImpl implements CMSOperations {
	 ...
	 */
	// add a ctor
	//collection is collection of references
	
	public CMSOperationsImpl() throws CMSException {
		this.customers = new ArrayList<>(10000);
...
//		for (Customer c : customers) {
//			if (c.getPlan() == myPlan) {
//				customers.remove(c);//instead of continuing 
...
//		}
		...
		while (customerItr.hasNext()) {
			...
			if (c.getPlan() == myPlan) {
				customerItr.remove();//remove the last element in the list
...
//core ,custom exception ,service ,ui,validations
